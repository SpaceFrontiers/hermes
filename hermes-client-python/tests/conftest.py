...
def _wait_for_server(proc, log):
    ...
    pytest.fail(f"hermes-server {reason}\nLast server output:\n{output}")


@pytest.fixture(scope="session")
def server():
    """Start hermes-server and release its process/files even if setup fails."""
    # Files avoid filling an undrained subprocess pipe during long tests.
    with (
        tempfile.TemporaryDirectory(prefix="hermes_test_") as data_dir,
...
async def client(server):
    ...
    async with HermesClient(SERVER_ADDRESS) as c:
        yield c
