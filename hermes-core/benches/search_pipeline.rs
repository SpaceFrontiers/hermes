...
fn bench_plain_ranked_text(c: &mut Criterion) {
    use hermes_core::query::{BooleanQuery, TopKCollector, collect_segment};
    ...
    for layout in ["early", "late", "mixed"] {
            ...
            for row in 0..32768 {
                ...
                for attempt in 0..=1000 {
                    match writer.add_document(document.clone()) {
                        Ok(()) => break,
                        Err(hermes_core::Error::QueueFull) if attempt < 1000 => {
        ...
        let queries: [(&str, Box<dyn Query>); 3] = [
            ...
            (
                "filter",
                Box::new(hermes_core::query::PrefixQuery::text(body, "alp")),
            ),
            (
                "and",
                Box::new(
        ...
        for (kind, query) in queries {
            group.bench_function(format!("{layout}/{kind}/count"), |b| {
                b.iter(|| {
                    let mut count = hermes_core::query::CountCollector::new();
                    runtime
...
fn bench_pattern_filters(c: &mut Criterion) {
    use hermes_core::query::{TopKCollector, collect_segment_with_limit_sync};
    ...
    let queries: [(&str, Box<dyn Query>); 3] = [
        (
            "regex",
            Box::new(hermes_core::RegexQuery::new(field, "g.*itar").unwrap()),
        ),
        (
            "single_star",
            Box::new(hermes_core::WildcardQuery::new(field, "g*itar").unwrap()),
        ),
        (
            "prefix",
            Box::new(hermes_core::PrefixQuery::text(field, "g000")),
        ),
    ];
...
fn bench_union_membership(c: &mut Criterion) {
    use hermes_core::query::{CountCollector, PrefixQuery, collect_segment};
    use hermes_core::segment::{SegmentBuilder, SegmentBuilderConfig, SegmentId, SegmentReader};
    let runtime = tokio::runtime::Builder::new_current_thread()
        .enable_all()
        .build()
...
fn bench_membership_density(c: &mut Criterion) {
    use hermes_core::query::{BooleanQuery, CountCollector, collect_segment};
    use hermes_core::segment::{SegmentBuilder, SegmentBuilderConfig, SegmentId, SegmentReader};
    let runtime = tokio::runtime::Builder::new_current_thread()
        .enable_all()
        .build()
