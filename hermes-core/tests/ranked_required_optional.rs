...
{
    use hermes_core::query::{ScorerOptions, collect_segment_with_limit};
    use hermes_core::structures::{PostingCodec, TERMINATED};
    for (codec, impacts) in [
        ...
        for (required, optional) in [
            ...
            for limit in [1, 10, 100, 1000] {
                ...
                let pairs = |hits: &[hermes_core::query::SearchResult]| {
                    hits.iter()
            ...
            let budget = hermes_core::query::SharedThreshold::for_limit(10)
...
async fn required_optional_windows_preserve_global_scores_positions_and_chunk_fallbacks() {
    ...
    use hermes_core::query::{GlobalStatsBuilder, SharedThreshold, search_segment_shared};
            ...
            for query in [
                ...
                for positions in [false, true] {
                    ...
                    for k in [0, 1, 10, 100, 1000] {
                        ...
                        #[cfg(feature = "sync")]
                        {
                            let sync = hermes_core::query::search_segment_shared_sync(
                                &reader,
                                &query,
                                k,
                                positions,
