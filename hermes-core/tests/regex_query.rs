...
use hermes_core::query::{CountCollector, collect_segment};
use hermes_core::segment::{SegmentBuilder, SegmentBuilderConfig, SegmentId, SegmentReader};
use hermes_core::{Document, QueryLanguageParser, RamDirectory, SchemaBuilder};
use std::sync::Arc;
...
async fn regex_queries_match_whole_terms_and_preserve_boolean_composition() {
    ...
    for (expression, expected) in [
        ...
        while scorer.doc() != hermes_core::TERMINATED {
...
async fn benchmark_regex_language_has_exact_case_sensitive_constant_score_results() {
    ...
    for &(pattern, expected) in cases {
        ...
        assert_eq!(scorer.doc(), hermes_core::TERMINATED, "{pattern}");
        let mut count = CountCollector::new();
        ...
        #[cfg(feature = "sync")]
        {
            ...
            assert_eq!(scorer.doc(), hermes_core::TERMINATED);
        }
    }
    use hermes_core::Query;
...
async fn regex_expansion_budget_fails_without_partial_results() {
    use hermes_core::{Query, RegexQuery};
    let mut schema = SchemaBuilder::default();
    let field = schema.add_text_field_with_tokenizer("tag", true, false, "raw");
    let schema = Arc::new(schema.build());
    let directory = RamDirectory::new();
...
async fn pattern_queries_reject_unindexed_nontext_and_unknown_fields() {
    use hermes_core::{Query, RegexQuery, WildcardQuery};
    let mut schema = SchemaBuilder::default();
    ...
    let reader = SegmentReader::open(&directory, id, schema, 16)
        .await
        .unwrap();
    for field in [stored, number, hermes_core::Field(999)] {
