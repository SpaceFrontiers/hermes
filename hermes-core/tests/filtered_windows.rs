...
async fn deleted_text_collection_matches_scalar_scores_counts_and_positions() {
    ...
    schema.set_positions(text, hermes_core::dsl::PositionMode::TokenPosition);
    ...
    for query in queries {
        ...
        while scalar.doc() != hermes_core::TERMINATED {
        ...
        let (hits, _) = hermes_core::query::search_segment_with_count(segment, query.as_ref(), 100)
            .await
            .unwrap();
        ...
        {
            let (hits, _) =
                hermes_core::query::search_segment_with_count_sync(segment, query.as_ref(), 100)
                    .unwrap();
            assert_eq!(hits, expected);
        }
    }
