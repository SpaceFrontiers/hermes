...
use hermes_core::directories::Directory;
use hermes_core::query::{
    BooleanQuery, CountCollector, PhraseQuery, Query, TermQuery, TopKCollector, collect_segment,
};
use hermes_core::{Document, Index, IndexConfig, IndexWriter, RamDirectory, Schema};
...
async fn independent_text_permutations_keep_cross_field_scores_and_positions() {
    ...
    let queries: Vec<Box<dyn Query>> = vec![
        ...
        Box::new(
            BooleanQuery::new()
                .must(TermQuery::text(left, "alpha"))
                .should(hermes_core::query::BoostQuery::new(
...
async fn reordered_composition_streams_blocks_without_replaying_logical_ids() {
    use hermes_core::query::collect_segment_with_limit;
    use hermes_core::search_diagnostics::capture;
    let mut schema = Schema::builder();
    ...
    for (query, count) in [
        ...
        hermes_core::query::collect_segment_with_limit_sync(segment, &query, &mut sync_top, 10)
            .unwrap();
        assert_eq!(
            sync_top
                .into_sorted_results()
    ...
    let expired = hermes_core::query::SharedThreshold::for_limit(10)
        .with_deadline(Some(std::time::Instant::now()));
    let (result, work) = capture(hermes_core::query::search_segment_shared(
        segment,
        &phrase,
        10,
        true,
...
async fn plain_text_rgb_preserves_scores_positions_and_stable_documents() {
    let schema = |reorder| {
        ...
        builder.set_positions(text, hermes_core::dsl::PositionMode::TokenPosition);
    ...
    for (query, (_, expected)) in queries.iter().zip(&expected) {
        for limit in [1, 10, 100, 1000] {
            ...
            hermes_core::query::collect_segment_with_limit(
...
async fn unchanged_payloads(directory: RamDirectory, config: IndexConfig) -> Vec<Vec<u8>> {
    ...
    let reader = index.reader().await.unwrap();
    let searcher = reader.searcher().await.unwrap();
    let files = hermes_core::segment::SegmentFiles::new(searcher.segment_readers()[0].meta().id);
...
async fn merge_time_text_budget_failure_keeps_sources_and_removes_unpublished_output() {
    ...
    let before = hermes_core::index::IndexMetadata::load(&dir).await.unwrap();
    let source_files = dir.list_files(std::path::Path::new("")).await.unwrap();
    let error = writer.force_merge().await.unwrap_err();
    assert!(error.to_string().contains("budget"), "{error}");
    let after = hermes_core::index::IndexMetadata::load(&dir).await.unwrap();
