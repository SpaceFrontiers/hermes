...
use hermes_core::dsl::{PositionMode, QueryLanguageParser};
use hermes_core::index::{Index, IndexConfig, IndexWriter};
use hermes_core::query::{Collector, CountCollector, Query, TopKCollector, collect_segment};
use hermes_core::structures::PostingCodec;
        ...
        while let Some(flag) = args.next() {
            match flag.as_str() {
                ...
                "--no-background-merges" => {
                    options.config.merge_policy = Box::new(hermes_core::merge::NoMergePolicy);
...
fn parse_query(parser: &QueryLanguageParser, text: &str) -> Result<Box<dyn Query>> {
    ...
    if text
        ...
        .filter(|s| !s.is_empty())
        .count()
        > hermes_core::query::MAX_QUERY_TERMS
    {
    ...
    for line in std::io::stdin().lock().lines() {
        ...
        loop {
            match writer.add_document(doc.clone()) {
                Ok(()) => break,
                Err(hermes_core::Error::QueueFull) if std::time::Instant::now() < deadline => {
...
async fn serve(path: &Path, options: Options) -> Result<()> {
    ...
    for line in std::io::stdin().lock().lines() {
        ...
        #[cfg(feature = "query-diagnostics")]
        {
            let (result, work) = hermes_core::search_diagnostics::capture(execution).await;
...
impl<C: Collector> Collector for Exhaustive<C> {
    fn collect(
        ...
        doc: u32,
        score: f32,
        positions: &[(u32, Vec<hermes_core::query::ScoredPosition>)],
    ) {
