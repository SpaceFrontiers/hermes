...
Apply each patch independently to Hermes `46f4e294` (1.9.1). An empty
...
RUSTFLAGS='-C target-cpu=native' cargo build --locked --release \
  -p hermes-server --example searchbench_http
```

The build must recompile `hermes-core` after each patch. Save distinct binary
