...
Same 684 successful query texts from the original 826-query corpus; 10M documents, 32 clients, 30 server CPU threads and two driver threads. These are successful-query throughput results, not a ranking-equivalence test. Hermes counts differ from the reference engines; [coverage](coverage.json), [all counts](counts.json), and [substantial count outliers](count-outliers.json) remain visible.
...
| Family             | Timed / original | Counts within 5% | Hermes | Hermes RGB | Elasticsearch | OpenSearch |   Luxir |
| ------------------ | ---------------: | ---------------: | -----: | ---------: | ------------: | ---------: | ------: |
...
The dashes mean no common successful-query timing exists. All three reference engines execute all 826 count probes; Hermes fails 142 explicit scan/expansion budgets. Missing successful-query timings must not be treated as zero cost or as a fast error response.
