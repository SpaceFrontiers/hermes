# Text index formats and memory: Hermes versus Tantivy
...
Hermes Rounded postings/POS3 positions, and Tantivy 0.26. Compare file bytes,
...
| Component         | Hermes current fixture                                                                                                                 | Tantivy 0.26                                                                |
| ----------------- | -------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------- |
...
These are the formats actually used here. Hermes also has explicit Packed,
...
Hermes: [posting layout](posting-codecs.md),
[position stream](../hermes-core/src/structures/postings/positions_v2.rs),
[length columns](../hermes-core/src/segment/chunk_map.rs).

## Admission also affects residency

On a structural-validation cache miss, Hermes checks every posting/position
...
advances through widths to skip payload bytes. Hermes's opt-in compact formats
...
See [posting admission](../hermes-core/src/structures/postings/posting/validation.rs),
[position admission](../hermes-core/src/structures/postings/positions_v2.rs),
[bounded proof ownership](../hermes-core/src/structures/postings/posting/reader.rs),
...
| Official workload, MiB | Selected Hermes | Tantivy | Difference |
| ---------------------- | --------------: | ------: | ---------: |
...
Hermes build has essentially the same index-file residency as the selected build.

Standalone terms have no position-file residency: selected Hermes RSS is
...
| Component                            | Hermes MiB | Tantivy MiB | Hermes / Tantivy |
| ------------------------------------ | ---------: | ----------: | ---------------: |
...
metadata explains **38.0%** of the position-file gap. Hermes's posting metadata
...
Hermes has 12,660,057 posting blocks, **67.1% short**, and 18,316,784 position
...
Hermes's ability to copy compatible encoded runs during merge.
...
estimates preserve Hermes's existing block boundaries and exclude replacement
...
The indexes retain their documented upstream analyzer differences: Hermes has
...
Tantivy's default analyzer excludes tokens of 40 bytes or more; Hermes retains
...
Tantivy table. Applying that table to Hermes's current saturated u16 lengths
...
- PISA's [SIMD-BP128 codec](https://github.com/pisa-engine/pisa/blob/master/include/pisa/codec/simdbp.hpp)
  ...
  best traversal or bound partition for Hermes.
...
against either engine. Hermes still needs bounded copy-compatible tails and
...
one-byte-per-document column. A Hermes experiment must compare both precision
...
Hermes POS3 records a 4-byte header and 12-byte physical/logical directory entry
