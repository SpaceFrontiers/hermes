...
Hermes improves by **1.42× for TOP_10**, **1.45× for TOP_1000**, and
**5.12× for exact COUNT** on the official workload. Exact counts agree with both
reference engines, but Hermes still trails Tantivy by 2.24–3.59× across the five
...
The main before/after runs reuse **identical persisted Hermes index bytes**.
...
Base Hermes revision is `ce2c96b945fccc4bac58ccc45bb4bc23b809773e`; the baseline
...
All 962 exact counts agree with both reference engines. Pruned Hermes top-10,
top-100 and top-1000 have identical scores and ordered document IDs to exhaustive
Hermes scoring. Every result below includes parsing and the local pipe round-trip;
...
Hermes remains slower overall than both reference engines. The main comparison
...
| Query family       | Queries | TOP_10 speedup | TOP_10 Hermes/Tantivy | COUNT speedup | COUNT Hermes/Tantivy |
| ------------------ | ------: | -------------: | --------------------: | ------------: | -------------------: |
...
| Command  | Exhaustive Hermes µs | Exhaustive/pruned latency |
| -------- | -------------------: | ------------------------: |
| TOP_10   |                2,904 |                     1.96× |
| TOP_100  |                3,055 |                     1.72× |
| TOP_1000 |                3,172 |                     1.52× |
...
| Command  | Final Hermes µs | Before/after speedup | Hermes/Tantivy latency |
| -------- | --------------: | -------------------: | ---------------------: |
...
| Workload  | Command       | Cache-1024 Hermes µs | Default/cache-1024 speedup |
| --------- | ------------- | -------------------: | -------------------------: |
...
| Command       | Hermes before | Hermes final | Hermes cache-1024 | Tantivy | Lucene |
| ------------- | ------------: | -----------: | ----------------: | ------: | -----: |
...
| Index          |         Bytes | Relative to original Hermes |
| -------------- | ------------: | --------------------------: |
| Hermes rounded | 5,322,696,537 |                      1.000× |
| Tantivy 0.26   | 3,029,870,002 |                      0.569× |
| Lucene 10.4.0  | 2,806,588,208 |                      0.527× |
| Hermes packed  | 4,800,242,810 |                      0.902× |
| Hermes pfor    | 4,772,404,644 |                      0.897× |

The original Hermes index uses 2,871,883,607 bytes for positions (54%) and
...
| Command       | Rounded Hermes µs | Packed Hermes µs | PFor Hermes µs |
| ------------- | ----------------: | ---------------: | -------------: |
...
The final result is not a claim that Hermes is the fastest engine. Profiles and
...
- **Block bounds:** the earlier MaxScore candidate's standalone-term diagnostic
  ...
  Hermes combines maximum TF and minimum length even when they belong to
...
- **Packed decoding:** the existing
  ...
  latency claim or new Hermes dependency; see the
  [evaluation design](search-benchmark-game.md#existing-library-decoder-candidate).
- **Positions:** positions occupy 54% of the original Hermes index. Reuse the
...
settings: Hermes/Tantivy use k1=1.2, b=0.75; Lucene's adapter uses k1=0.9, b=0.4.
Tantivy drops tokens longer than 40 characters; Lucene splits overlong tokens at
255; Hermes retains them. Norm quantization also differs. These differences are
recorded in the [protocol](search-benchmark-game.md#analyzer-and-scoring-differences),
so cross-engine ranking equivalence is not claimed. Within Hermes, the exactness
gate compares ordered document identities and `f32::to_bits()` at all three
ranked depths against exhaustive scoring.

Baseline Hermes had unsafe saturated TF bounds and floating-point ranking
...
and is not equivalent to heap allocation or cache capacity. Hermes/Tantivy
...
original Hermes indexing process peaked at 14,099,616 KiB including the final
...
`hermes-core/examples/search_benchmark_game.rs`. See the
...
`.context/hermes-benchmark-evidence.tar.gz`, verified SHA-256
...
`.context/hermes-baseline.bundle` and `.context/native-parser-baseline-v4.tar.gz`
