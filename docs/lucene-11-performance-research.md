# Lucene 11 performance research for Hermes
...
The useful lesson for Hermes is to move work through contiguous batches and
...
| Mechanism                                       | Source and status                                                                                                                                                                                                                                                                                                | Hermes implication                                                                                                                                                                                                                                                                                                                             |
| ----------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
| Batch disjunction iteration                     | [PR #16394](https://github.com/apache/lucene/pull/16394), development work listed for 10.6.                                                                                                                                                                                                                      | The same principle as Hermes's membership windows: avoid a heap/cursor update for every match. Useful for constant-score filters as well as exhaustive counts. Maintain an independent scalar oracle.                                                                                                                                          |
| Lazy MaxScore scratch                           | [PR #16316](https://github.com/apache/lucene/pull/16316), development work.                                                                                                                                                                                                                                      | Hermes's previous single-term specialization already avoids general window buffers. Keep scratch lazy on paths that actually use it; do not allocate per-term dense arrays for paths that only need membership.                                                                                                                                |
...
explain single-thread uncached BM25 execution speed. Hermes's decompressed term
...
dense vectors. Hermes already executes native Rust SIMD kernels, and sparse
...
Hermes retained bulk score windows, contiguous term-run BM25, and standalone
...
Hermes currently compresses approximately 16 KiB SSTable blocks and includes
...
Hermes still takes 2.338× Tantivy's time overall for that command. The matched
...
division per hit. Hermes's current canonical calculation divides both length by
...
Hermes's current scores. A separate, unimplemented option is a bounded lookup
...
without measuring Hermes's generated code would be premature.
...
checks for new bytes. It leaves Hermes 2.11–2.60× slower than Tantivy across those
...
Hermes STB5/Zstd blocks inspired by Lucene's block-tree granularity, with equal
...
found in how Hermes supplied its safety limit to the existing Zstd API; that fix
is measured independently. These are Hermes changes informed by the research,
...
Hermes's ordinary iterator previously decoded frequencies on every document
...
[Hermes validation and measurement record](search-performance-review.md#deferred-frequency-experiment-mixed-full-corpus-result)
...
optimizations. Hermes's compact conjunction now sorts by document frequency;
...
Hermes's ordinary posting iterator already uses a nearby probe and suffix binary
...
empty. Hermes's preceding scorer reads all term positions before checking their
...
Hermes currently uses dynamically composed complete streams for this shape.
The proposed required text windows reuse Hermes's existing batched executor,
