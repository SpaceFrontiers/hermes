...
impl Envelope {
    pub fn from_value(mut value: Value) -> Result<Self> {
        ...
        if text.split_whitespace().count() > hermes_core::query::MAX_QUERY_TERMS {
    ...
    pub fn parse(
        ...
        let query: Box<dyn Query> = match self.class.as_str() {
            ...
            "high_sloppy_phrase" | "med_sloppy_phrase" | "low_sloppy_phrase" => {
                ...
                if tokens.len() > hermes_core::query::MAX_QUERY_TERMS {
...
mod tests {
    ...
    fn envelope_preserves_validation_order_and_limits() {
        for (value, error) in [
            ...
            (
                json!({"query": "a ".repeat(hermes_core::query::MAX_QUERY_TERMS + 1), "class": "high_term", "limit": 0}),
